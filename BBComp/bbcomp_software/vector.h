...
public:
	...
	Vector(double scalar);                      // zero-dimensional "vector" with given value
	...
	// apply permutation
	Vector permute(std::vector<unsigned int> const& permutation) const;

	// element-wise operations
	Vector elemProduct(Vector const& rhs) const;
	Vector elemQuotient(Vector const& rhs) const;
	Vector elemPower(double const& rhs) const;
	Vector elemPower(Vector const& rhs) const;
	...
	// dominance relation
	// note that the relation is not complete, hence (a < b) is not the contrary of (b >= a)
	bool operator < (Vector const& rhs) const;
	bool operator <= (Vector const& rhs) const;
	bool operator > (Vector const& rhs) const
	{ return (rhs < *this); }
